"""
This file generates captions on the COCO dataset images using BLIP.
It was used to obtain the COCO benchmarks for the BLIP model.
...
def main(n_images=50, set_type='test', lm_model_params=None):
    """
    The default parameters were used to generate the BLIP captions on the COCO dataset for the benchmarking against
    ...
    for img_path, img in image_captioner.img_dic.items():
        # Generate a caption for the image with BLIP
        caption = blip_manager.generate_response(img, model_params=lm_model_params)
        # Add the data sample
        data_list.append({
    ...
    file_path = f'../data/outputs/captions/blip_caption_{set_type}.csv'
