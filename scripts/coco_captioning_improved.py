...
class ImageCaptioner:
    ...
    @print_time_dec
    def main(
            self, num_images=50, num_captions=10, lm_temperature=0.9, lm_max_length=40, lm_do_sample=True, cos_sim_thres=0.7,
            num_objects=5, num_places=2, caption_strategy='baseline', set_type='train'
    ):
        """
        ...
        for img_file in img_files:
            # Load the image
            img_dic[img_file] = self.image_manager.load_image(self.coco_manager.image_dir + img_file)
            # Generate the CLIP image embedding
            img_feat_dic[img_file] = self.clip_manager.get_img_emb(img_dic[img_file]).flatten()
        ...
        pg_map = {
            'baseline': self.prompt_generator.create_baseline_lm_prompt2,
        ...
        for img_name in img_dic:
            ...
            sorted_captions, caption_scores = self.clip_manager.get_nn_text(
                caption_texts, caption_emb, img_feat_dic[img_name]
            )
    ...
    def random_parameter_search(self, n_rounds, template_params):
        for _ in range(n_rounds):
            ...
            self.main(**template_params, set_type='train')
...
if __name__ == '__main__':
    ...
    image_captioner.random_parameter_search(n_rounds=50, template_params=template_params)
