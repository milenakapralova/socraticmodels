...
if __name__ == '__main__':
    ...
    file_list = [f for f in os.listdir(caption_dir) if f.endswith('csv') and 'train' in f]
    for f in file_list:
        str_split = f.split('_')
        run_params = {
            'n_captions': 50,
            'lm_temperature': float(f'0.{str_split[5][1:]}'),
            'lm_max_length': 40,
            'lm_do_sample': True,
            'cos_sim_thres': float(f'0.{str_split[3][1:]}'),
