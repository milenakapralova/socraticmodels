...
try:
    ...
    pass
from scripts.image_captioning_multiple_prompts import (
...
def main(
        ...
        num_objects=5, num_places=2, caption_strategy='multiple_prompts', random_seed=42
):
    """
    1. Set up
    """
    ...
    pg_map = {
        ...
        'multiple_prompts': prompt_generator.create_multiple_prompts,
    }
    ...
    for img_name in img_dic:
        ...
        if caption_strategy == 'multiple_prompts':
            
            caption_texts_list = []
            caption_emb_list = None
            for prompt in prompt_dic[img_name]:
                caption_texts = flan_manager.generate_response(num_captions * prompt, model_params)
                caption_texts_list += caption_texts

                # Zero-shot VLM: rank captions.
                caption_emb = clip_manager.get_text_emb(caption_texts)

                if caption_emb_list is None:
                    caption_emb_list = caption_emb
                else:
                    caption_emb_list = np.vstack([caption_emb_list, caption_emb])
            caption_texts = caption_texts_list
            caption_emb = caption_emb_list
